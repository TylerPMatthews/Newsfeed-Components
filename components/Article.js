...
function articleMaker({
  ...
  //Create the structure of the article maker
  ...
  //set the classes of each part of the component
  articleMain.classList.add("article");
  articleDate.classList.add("date");
  mySpan.classList.add("expandButton");
  //add a on click event listener for the span button
  mySpan.addEventListener("click", function (event) {
    articleMain.classList.toggle("article-open");
  });
  //always make sure to return something
  return articleMain;
}
//map through the data given above and pass it through the articleMaker
let articleElement = data.map((data) => {
  return articleMaker(data);
});
//Loop through the data given from our Map and append it to the DOM!
